    ...
    <div className="mt-[5rem] grid grid-cols-1 xl:grid-cols-3 gap-6 md:grid-cols-2">
      {services.map((service, index) => {
        return (
          <div
            key={index}
            className="relative p-10 flex flex-col text-white border border-color-1 rounded-xl"
          >
              ...
              <ul className="pt-6 flex flex-col gap-3">
                {service.bulletPoints.map((point, index) => {
                  return (
                    <li
                      key={index}
                      className="flex items-center gap-3  text-gray-300 transition-transform hover:scale-110 cursor-pointer"
                    >
                      {check}
                      <span>{point}</span>
                    </li>
            ...
            <span
              className="absolute top-0 left-0 bg-4 text-3xl font-semibold p-2 rounded-tr-[30px] rounded-bl-[30px] 
